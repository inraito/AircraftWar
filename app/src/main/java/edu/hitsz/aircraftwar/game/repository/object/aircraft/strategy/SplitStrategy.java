...
public class SplitStrategy implements ShootStrategy {
    ...
    public SplitStrategy(int direction,int shootNum, int power){
    ...
    public List<AbstractBullet> shoot(AbstractAircraft context) {
        ...
        for(int i=0; i<shootNum; i++){
            ...
            if(context instanceof HeroAircraft) {
                baseBullet = FlyingObjectFactoryManager.getBulletFactory().getHeroBullet(x + (i * 2 - shootNum + 1) * 10, y,
                        speedX + (i * 2 - shootNum + 1), speedY
                        , power);
            }else{
                baseBullet = FlyingObjectFactoryManager.getBulletFactory().getEnemyBullet(x + (i * 2 - shootNum + 1) * 10, y,
                        speedX + (i * 2 - shootNum + 1), speedY
                        , power);
            }
            res.add(baseBullet);
